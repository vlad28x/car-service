...
public class OrderMapper implements Mapper<OrderEntity, OrderRequestDto, OrderResponseDto> {
    ...
    public OrderMapper(OrderStatusRepository orderStatusRepository, ServiceRepository serviceRepository,
                       UserRepository userRepository, OrderStatusMapper orderStatusMapper,
                       ServiceMapper serviceMapper, UserMapper userMapper) {
    ...
    public OrderResponseDto toResponseDto(OrderEntity entity) {
        ...
        responseDto.setServices(Optional.ofNullable(entity.getServices()).map(services ->
                services.stream().map(serviceMapper::toResponseDto).collect(Collectors.toList())).orElse(null));
        responseDto.setWorker(Optional.ofNullable(entity.getWorker()).map(userMapper::toResponseDto)
                .orElse(null));
        responseDto.setManager(Optional.ofNullable(entity.getManager())
                .map(userMapper::toResponseDto).orElse(null));
        responseDto.setCustomer(Optional.ofNullable(entity.getCustomer()).map(userMapper::toResponseDto)
                .orElse(null));
        return responseDto;
    }
    ...
    private List<ServiceEntity> getServices(List<Long> servicesId) {
        return Optional.ofNullable(servicesId)
                .map(services -> services.stream().map(serviceRepository::findById).filter(Optional::isPresent)
                        .map(Optional::get)
